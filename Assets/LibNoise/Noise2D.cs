...
namespace LibNoise
{
    ...
    public class Noise2D : IDisposable
    {
        ...
        {
            var texture = new Texture2D(_width, _height, TextureFormat.ARGB32, true, false);
        ...
        public Texture2D GetNormalMap(float intensity)
        {
            ...
            var pixels = new Color[_width * _height];
            Debug.Log(_width + " " +_height + " " + Border);
            Debug.Log(_ucWidth + " " + _ucHeight + " " + _ucBorder);
            for (var x = 0; x < _ucWidth; x++)
            {
                for (var y = 0; y < _ucHeight; y++)
                {
                    ...
                    var yPos = (_ucData[x, Mathf.Max(0, y - _ucBorder)] -
                                    _ucData[x, (Mathf.Min(y + _ucBorder, _width + _ucBorder)) - 1])/2;
